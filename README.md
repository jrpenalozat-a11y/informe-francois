...
3. **Informe de vuelta** (al pie de Generar) — pegas ahí el texto que devolvió Claude, **lo corriges y lo mandas**. Incluye:
   ...
   - **🖨️ PDF**: arma una hoja con el título (modo + período), el informe ya formateado y, abajo, las fotos marcadas con su día · hora · área y el texto de su nota. Se abre el diálogo de imprimir: en el teléfono, «Guardar como PDF». Ese archivo sí se adjunta al grupo o se lleva a la reunión.
...
- `sw.js` — service worker. Constante `CACHE` (hoy `if-v12`): **súbela** al cambiar assets del núcleo. HTML network-first, el resto cache-first. Las llamadas a Supabase **no pasan por la caché** (si no, la app leería siempre la misma respuesta vieja).
