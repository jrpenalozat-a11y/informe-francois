...
1. **📝 Notas** — agregas la nota al paso: fecha, hora, área (Sala · Cocina · Caja · Personal · Proveedores · Equipos · Producto · Otro), **qué pasó** y, opcional, la **medida tomada en el momento**. Se pueden editar y borrar; se agrupan por día. `Ctrl + Enter` guarda rápido.
   ...
   **📷 Fotos** — cada nota admite fotos: **Tomar foto** abre la cámara del teléfono y **Desde la galería** toma una ya sacada (varias a la vez). Se comprimen a 1600 px / JPEG antes de guardarlas (una foto de teléfono queda en ~100-250 KB), aparecen como miniaturas en el formulario y en la nota, y se abren en el visor: pasar entre fotos, **Compartir** (mandarla por WhatsApp desde el teléfono), **Guardar** en el dispositivo o **Borrar**. Si aprietas «Agregar nota» mientras la foto se está guardando, la nota se guarda sola apenas termina.
...
3. **Informe de vuelta** (al pie de Generar) — pegas ahí el texto que devolvió Claude, **lo corriges y lo mandas**. Incluye:
   ...
   - **🖨️ PDF**: arma una hoja con el título (modo + período) y el informe ya formateado, con **cada foto justo bajo el punto que la menciona** (no todas juntas al final). Como el informe lo redacta la IA con sus propias palabras, la app empareja cada nota con la línea que más se le parece: palabras en común, el «(hay foto)» de la regla 10 y el título del bloque (SALA, CAJA…). Las fotos de una misma nota van juntas, y las que no calzan con ninguna línea caen en **«Otras fotos»** al final, con su día · hora · área y el texto de la nota — así ninguna se pierde. Se abre el diálogo de imprimir: en el teléfono, «Guardar como PDF». Ese archivo sí se adjunta al grupo o se lleva a la reunión.
...
- `sw.js` — service worker. Constante `CACHE` (hoy `if-v13`): **súbela** al cambiar assets del núcleo. HTML network-first, el resto cache-first. Las llamadas a Supabase **no pasan por la caché** (si no, la app leería siempre la misma respuesta vieja).
